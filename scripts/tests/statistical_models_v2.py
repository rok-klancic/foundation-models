...
# DESCRIPTION
# This script is used to test the statistical models.
# However it predicts the diffs and then adds them to the real data.
# In this way it predicts the real values instead of the diffs.

# The difference to the previous script -- statistical_models.py -- is that that one predicts the target feature directly.
# We were using is for predicting the diffs.
# Also this script wants data that contains the diffs for every horizon, while the previous one wanted only one diff column.

# It is designed so that the target feature is the real value and then it uses the diffs


# CONSTANTS
# ----------------------------------------------------------------------------------------------------------------------
DATA_PATH = 'data/processed/ground-water-and-weather-with-forecasts-shifts-and-diffs.joblib'
ROOT_PATH = '../../'
...
def check_target_feature(target_feature, aquifer_by_stations, horizon_max, aquifers_list):
    features_to_check = [target_feature]

    # Add the diffs to the features to check
    for horizon in range(1, horizon_max + 1):
        features_to_check.append(f'{target_feature}_diff_{horizon}')

    for aquifer in aquifers_list:
        if  not all(elt in aquifer_by_stations[aquifer].columns for elt in features_to_check):
            return False
    return True

def convert_diffs_to_real_values(forecast, real_values):
    forecast = np.array(forecast)
    real_values = np.array(real_values)

    # Convert the forecasted diffs to the real values
    output = forecast + real_values
    ...
    def objective(trial):
        ...
        for aquifer in aquifers_list:
            ...
            for horizon in range (1, horizon_max+1, 1):
                ...
                # Define the target diff (target feature is the real value used only for testing)
                target_diff = f'{target_feature}_diff_{horizon}'
                
                # Define the train and test set
                X_train = aquifer_by_stations[aquifer][chosen_features][:-(val_len + horizon + test_len)]
                y_train = aquifer_by_stations[aquifer][target_diff][:-(val_len + horizon + test_len)]
        
                X_test = aquifer_by_stations[aquifer][chosen_features][-(val_len + horizon + test_len):-(horizon + test_len)]
                y_test = aquifer_by_stations[aquifer][target_diff][-(val_len + horizon + test_len):-(horizon + test_len)]
...
def k_best_feature_selection(aquifers_list, test_len, horizon_max, target_feature, aquifer_by_stations, k):
    ...
    for aquifer in aquifers_list:
        ...
        for horizon in range(1, horizon_max + 1):
            ...
            y = aquifer_by_stations[aquifer][target_diff][:-(test_len + horizon)]
    ...
    for aquifer in aquifers_list:
        ...
        for horizon in range (1, horizon_max+1, 1):
            ...
            if model_name != 'gradient_boosting':
                ...
                y_train = aquifer_by_stations[aquifer][target_diff][:-(test_len + horizon)]
                ...
                y_val = aquifer_by_stations[aquifer][target_diff][-(val_len + horizon + test_len):-(horizon + test_len)]
            ...
            # Convert the forecasted diffs to the real values
            forecast = convert_diffs_to_real_values(forecast, aquifer_by_stations[aquifer][target_feature][-(test_len + horizon):-horizon])
...
# Load the data
aquifer_by_stations = joblib.load(os.path.join(ROOT_PATH, DATA_PATH))
...
for name, settings in experiment_settings.items():
    ...
    # Check if the dataset contains the target feature and its diffs!
    if check_target_feature(target_feature=settings['target_feature'], 
                            aquifer_by_stations=aquifer_by_stations, 
                            horizon_max=settings['horizon_max'], 
                            aquifers_list=settings['aquifers_list']):
        pass
    else:
        raise ValueError(f"The dataset does not contain the target feature and its diffs! {settings['target_feature']}")
    ...
    if settings['additional_features']:
        ...
        for horizon in range(1, settings['horizon_max'] + 1):
            best_features[f'horizon_{horizon}'] = find_target_feature_columns(f'{settings['target_feature']}_diff_{horizon}', aquifer_by_stations)
