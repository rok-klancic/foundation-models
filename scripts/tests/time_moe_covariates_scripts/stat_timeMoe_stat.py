...
# STATIC VARIABLES
# ----------------------------------------------------------------------------------------------------------------------
TEST_LEN_MULTIPLIER = 5

# HYPERPARAMETER TUNING
# ----------------------------------------------------------------------------------------------------------------------

# Hyperparameter tuning for the initial statistical model
def hyperparameter_tuning_initial(model_name,
                          horizon_max,
                          aquifers_list,
                          best_features,
                          target_feature,
                          ...
                          test_len,
                          aquifer_by_stations):
    # Make test_len greater (5x) so the next model has more data to train on
    test_len = test_len * TEST_LEN_MULTIPLIER
...
# Hyperparameter tuning for the final statistical model (on prediction data)
def hyperparameter_tuning_final(model_name,
                          horizon_max,
                          aquifers_list,
                          target_feature,
                          val_len,
    ...
    def objective(trial):
        ...
        for aquifer in aquifers_list:
            ...
            for horizon in range (1, horizon_max+1, 1):
                ...
                y_train = temp['target'][horizon:-(val_len + test_len)]
                
                X_test = temp[['a', 'b']][-(val_len + test_len + horizon):-(horizon + test_len)]
                y_test = temp['target'][-(val_len + test_len):-test_len]
...
# INITIAL PREDICTION (statistical model)
# ----------------------------------------------------------------------------------------------------------------------
def prediction_initial(model_name,
                   aquifers_list,
                   test_len,
                   val_len,
                   horizon_max,
    ...
    for aquifer in aquifers_list:
        ...
        for horizon in range (1, horizon_max+1, 1):
            ...
            if model_name != 'gradient_boosting':
                ...
                X_val = temp[['a', 'b']][-(val_len + test_len + horizon):-(horizon + test_len)]
                y_val = temp['target'][-(val_len + test_len):-test_len]
